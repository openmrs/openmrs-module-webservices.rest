...
public class CohortControllerTest extends BaseModuleWebContextSensitiveTest {
	...
	@Test
	public void getCohort_shouldGetADefaultRepresentationOfACohort()
			throws Exception {
		Object result = controller.retrieve(cohortUUID, request);
		Assert.assertNotNull(result);
		log("Cohort fetched (default)", result);
		Assert.assertEquals(cohortUUID, PropertyUtils.getProperty(result,
				"uuid"));
	}

	@Test
	public void getCohortByName_shouldGetADefaultRepresentationOfACohort()
			throws Exception {
	...
	public void updateCohort_shouldChangeAPropertyOnACohort() throws Exception {
		...
		SimpleObject post = new ObjectMapper().readValue(json,
				SimpleObject.class);
		Object editedCohort = controller.update(cohortUUID, post, request,
				response);
		log("Edited cohort", editedCohort);
		Assert.assertEquals("EXTRA COHORT", service.getCohortByUuid(cohortUUID)
				.getName());
	}

	@Test
	public void getCohortMembers_shouldReturnListOfCohortMembers()
			throws Exception {
		int membersCount = service.getCohortByUuid(cohortUUID).getMemberIds()
				.size();
		List<Object> members = controller
				.getCohortMemebers(cohortUUID, request);
