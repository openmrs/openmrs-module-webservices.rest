...
public class HelpController {
	...
	public void showPage(ModelMap map, HttpServletRequest request)
		...
		List<SearchHandlerDoc> searchDocs = ResourceDocCreator
				.createSearchHandlerDoc(url);
		...
		for (ResourceDoc resourceDoc : docs) {
			for (int i = 0; i < resourceDoc.getSupportedOpenMRSVersion().size(); i++)
				if (!distinctOpenMRSSupportedVersionList.contains(resourceDoc
						.getSupportedOpenMRSVersion().get(i)))
					distinctOpenMRSSupportedVersionList.add(resourceDoc
							.getSupportedOpenMRSVersion().get(i));
		}
	...
	public void showPageWithFiltersOnVersions(
			...
			Errors errors, ModelMap map, HttpServletRequest request)
			throws IllegalAccessException, InstantiationException, IOException,
			ConversionException {
		...
		List<String> distinctOpenMRSSupportedVersionList = documentationConfiguration
				.getSelectedVersions();

		List<ResourceDoc> filteredDocs = new ArrayList<ResourceDoc>();

		for (ResourceDoc temp : docs) {
			for (int i = 0; i < temp.getSupportedOpenMRSVersion().size(); i++)
				if (distinctOpenMRSSupportedVersionList.contains(temp
						.getSupportedOpenMRSVersion().get(i))) {
