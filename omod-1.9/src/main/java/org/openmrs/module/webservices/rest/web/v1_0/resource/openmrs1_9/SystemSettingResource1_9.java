 ...
 * operations
 */
@Resource(name = RestConstants.VERSION_1 + "/systemsetting", order = 100, supportedClass = GlobalProperty.class, supportedOpenmrsVersions = {
		"1.9.*", "1.10.*", "1.11.*", "1.12.*" })
public class SystemSettingResource1_9 extends
		DelegatingCrudResource<GlobalProperty> {
	...
	/**
	 * Fetches a global property by uuid, if no match is found, it tries to look
	 * up one with a matching name with the assumption that the passed parameter
	 * is a global property name
	 *
	...
	public GlobalProperty getByUniqueId(String uuid) {
		...
		if (gp == null) {
			...
			gp = Context.getAdministrationService().getGlobalPropertyObject(
					uuid);
		}
	 ...
	 */
	@Override
	protected void delete(GlobalProperty property, String reason,
	 ...
	 */
	@Override
	public void purge(GlobalProperty property, RequestContext context)
	...
	protected NeedsPaging<GlobalProperty> doGetAll(RequestContext context) {
		List<GlobalProperty> allGlobalPropertys = Context
	...
	protected PageableResult doSearch(RequestContext context) {
		...
		searchResults = service.getGlobalPropertiesByPrefix(context
				.getParameter("q"));
		PageableResult result = new NeedsPaging<GlobalProperty>(searchResults,
				context);
		return result;
	}
	...
	public static String getDisplayString(GlobalProperty globalProperty) {
		...
		return property.getSection() + " - " + property.getName() + " = "
	 ...
	 *
	 * @param instance
	 *            the delegate instance to get the value off
	 * @return value object
	 */
	...
	public static void setValue(GlobalProperty property, String value)
			throws Exception {
		if (StringUtils.isNotEmpty(property.getDatatypeClassname())) {
			CustomDatatype customDataType = CustomDatatypeUtil.getDatatype(
					property.getDatatypeClassname(),
			...
			if (customDataType != null) {
				try {
					...
					throw new APIException(
							"Exception in converting value to custom data type",
							ex);
				}
			} else
				throw new APIException(
						"Custom data type is null as per provided parameters");
		} else {
