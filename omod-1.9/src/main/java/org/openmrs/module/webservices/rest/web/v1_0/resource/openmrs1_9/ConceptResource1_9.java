...
/**
 * Implemented to increase the resource version since the mapping subresource
 * changed.
 */
@Resource(name = RestConstants.VERSION_1 + "/concept", order = 100, supportedClass = Concept.class, supportedOpenmrsVersions = {
		"1.9.*", "1.10.*" })
public class ConceptResource1_9 extends ConceptResource1_8 {
	...
	public Concept getByUniqueId(String identifier) {
		...
		if (identifier.contains(":")) {
			...
			concept = Context.getConceptService().getConceptByMapping(termCode,
					sourceName, true);
		} else {
