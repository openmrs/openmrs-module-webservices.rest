...
@Resource(name = RestConstants.VERSION_1 + "/form", supportedClass = Form.class, order = 10, supportedOpenmrsVersions = {
        "1.9.* - 9.*" })
public class FormResource1_9 extends MetadataDelegatingCrudResource<Form> {
	...
	public Model getGETModel(Representation rep) {
		...
		if (rep instanceof DefaultRepresentation || rep instanceof FullRepresentation) {
			modelImpl.property("uuid", new StringProperty()).property("display", new StringProperty())
			        ...
			        .property("version", new StringProperty()).property("build", new IntegerProperty())
			        .property("published", new BooleanProperty()._default(false)).property("retired", new BooleanProperty());
		}
		if (rep instanceof DefaultRepresentation) {
			modelImpl.property("encounterType", new RefProperty("#/definitions/EncountertypeGetRef")).property("formFields",
			    new ArrayProperty(new RefProperty("#/definitions/FormFormfieldGetRef")));
		} else if (rep instanceof FullRepresentation) {
			modelImpl.property("encounterType", new RefProperty("#/definitions/EncountertypeGet")).property("formFields",
			    new ArrayProperty(new RefProperty("#/definitions/FormFormfieldGet")));
		}
		return modelImpl;
	}
	...
	public Model getCREATEModel(Representation rep) {
		ModelImpl model = ((ModelImpl) super.getCREATEModel(rep)).property("version", new StringProperty())
		        .property("encounterType", new StringProperty()).property("build", new IntegerProperty())
		        .property("published", new BooleanProperty()._default(false))
		        .property("formFields", new ArrayProperty(new StringProperty())).property("xslt", new StringProperty())
		        .property("template", new StringProperty())
		        
		        .required("version");
		if (rep instanceof FullRepresentation) {
			model.property("encounterType", new RefProperty("#/definitions/EncountertypeCreate")).property("formFields",
			    new ArrayProperty(new RefProperty("#/definitions/FormFormfieldCreate")));
		}
		return model;
	}
