...
@Resource(name = RestConstants.VERSION_1 + "/user", supportedClass = UserAndPassword1_9.class, supportedOpenmrsVersions = {
        "1.9.* - 1.12.*" })
public class UserResource1_9 extends MetadataDelegatingCrudResource<UserAndPassword1_9> {
	...
	public UserResource1_9() {
		
	}
	
	@RepHandler(RefRepresentation.class)
	public SimpleObject asRef(UserAndPassword1_9 delegate) throws ConversionException {
	 ...
	 */
	@Override
	public UserAndPassword1_9 newDelegate() {
		return new UserAndPassword1_9();
	}
	...
	public UserAndPassword1_9 save(UserAndPassword1_9 user) {
		...
		return new UserAndPassword1_9(openmrsUser);
	 ...
	 */
	@Override
	public UserAndPassword1_9 getByUniqueId(String uuid) {
		return new UserAndPassword1_9(Context.getUserService().getUserByUuid(uuid));
	}
	 ...
	 */
	@Override
	public void delete(UserAndPassword1_9 user, String reason, RequestContext context) throws ResponseException {
		if (user.isRetired()) {
	 ...
	 */
	@Override
	public void purge(UserAndPassword1_9 user, RequestContext context) throws ResponseException {
		if (user == null) {
			// DELETE is idempotent, so we return success here
			return;
		}
	 ...
	 */
	@Override
	protected NeedsPaging<UserAndPassword1_9> doSearch(RequestContext context) {
		// determine roles 
		...
		final List<UserAndPassword1_9> usersResult = new ArrayList<UserAndPassword1_9>();
		for (User user : users) {
			usersResult.add(new UserAndPassword1_9(user));
		}
		
		return new NeedsPaging<UserAndPassword1_9>(usersResult, context);
	}
	 ...
	 */
	@Override
	public Object getProperty(UserAndPassword1_9 instance, String propertyName) throws ConversionException {
		try {
	...
	public void setProperty(Object instance, String propertyName, Object value) throws ConversionException {
		try {
			UserAndPassword1_9 userAndPassword = (UserAndPassword1_9) instance;
			if (propertyName.equals("password")) {
	 ...
	 * @see User#getRoles()
	 */
	public Set<Role> getRoles(UserAndPassword1_9 user) {
	 ...
	 */
	public Set<Role> getAllRoles(UserAndPassword1_9 user) {
		if (user.getUser().getRoles() == null)
			return null;
	...
	public String getDisplayString(UserAndPassword1_9 user) {
	 ...
	 */
	@Override
	protected String getUniqueId(UserAndPassword1_9 delegate) {
	...
	public SimpleObject getAuditInfo(UserAndPassword1_9 delegate) throws Exception {
	 ...
	 */
	@Override
	protected NeedsPaging<UserAndPassword1_9> doGetAll(RequestContext context) {
		List<UserAndPassword1_9> users = new ArrayList<UserAndPassword1_9>();
		for (User user : Context.getUserService().getAllUsers()) {
			users.add(new UserAndPassword1_9(user));
		}
		return new NeedsPaging<UserAndPassword1_9>(users, context);
	}
}
