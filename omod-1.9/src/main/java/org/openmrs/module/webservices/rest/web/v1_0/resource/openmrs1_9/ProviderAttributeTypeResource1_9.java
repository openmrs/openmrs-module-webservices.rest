 ...
 */
@Resource(name = RestConstants.VERSION_1 + "/providerattributetype", order = 100, supportedClass = ProviderAttributeType.class, supportedOpenmrsVersions = {
		"1.9.*", "1.10.*", "1.11.*", "1.12.*" })
public class ProviderAttributeTypeResource1_9 extends
		BaseAttributeTypeCrudResource1_9<ProviderAttributeType> {
	 ...
	 */
	@Override
	protected NeedsPaging<ProviderAttributeType> doGetAll(RequestContext context)
			throws ResponseException {
		return new NeedsPaging<ProviderAttributeType>(service()
				.getAllProviderAttributeTypes(), context);
	}
	 ...
	 */
	@Override
	public void purge(ProviderAttributeType delegate, RequestContext context)
			throws ResponseException {
	...
	protected NeedsPaging<ProviderAttributeType> doSearch(RequestContext context) {
		// TODO: Should be a ProviderAttributeType search method in
		// ProviderService
		List<ProviderAttributeType> allAttrs = service()
		...
		for (ProviderAttributeType pAttr : allAttrs) {
			if (Pattern
					.compile(Pattern.quote(context.getParameter("q")),
							Pattern.CASE_INSENSITIVE).matcher(pAttr.getName())
					.find()) {
