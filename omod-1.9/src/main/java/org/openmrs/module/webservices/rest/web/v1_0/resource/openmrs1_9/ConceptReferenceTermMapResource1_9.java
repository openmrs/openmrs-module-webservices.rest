...
/**
 * {@link Resource} for {@link ConceptReferenceTermMap}, supporting standard
 * CRUD operations
 */
@Resource(name = RestConstants.VERSION_1 + "/conceptreferencetermmap", order = 100, supportedClass = ConceptReferenceTermMap.class, supportedOpenmrsVersions = {
		"1.9.*", "1.10.*", "1.11.*", "1.12.*" })
public class ConceptReferenceTermMapResource1_9 extends
		DelegatingCrudResource<ConceptReferenceTermMap> {
	...
	public String getDisplayString(
		...
		if (conceptReferenceTermMap.getTermA() == null
		...
		return conceptReferenceTermMap.getTermA().getConceptSource().getName()
				...
				+ " - "
				+ conceptReferenceTermMap.getTermB().getConceptSource()
						.getName() + ": "
				+ conceptReferenceTermMap.getTermB().getCode();
	}

	public SimpleObject getAuditInfo(ConceptReferenceTermMap delegate)
			throws Exception {
		...
		ret.put("changedBy", ConversionUtil.getPropertyWithRepresentation(
				delegate, "changedBy", Representation.REF));
		ret.put("dateChanged",
				ConversionUtil.convertToRepresentation(
						delegate.getDateChanged(), Representation.DEFAULT));
		return ret;
	}
	 ...
	 */
	@Override
	public ConceptReferenceTermMap save(
			ConceptReferenceTermMap conceptReferenceTermMap) {
	...
	public ConceptReferenceTermMap getByUniqueId(String uuid) {
		return Context.getService(RestHelperService.class).getObjectByUuid(
				ConceptReferenceTermMap.class, uuid);
	}
	 ...
	 */
	@Override
	public void purge(ConceptReferenceTermMap conceptReferenceTermMap,
	 ...
	 */
	@Override
	protected NeedsPaging<ConceptReferenceTermMap> doGetAll(
	 ...
	 */
	@Override
	protected void delete(ConceptReferenceTermMap delegate, String reason,
			RequestContext context) throws ResponseException {
		throw new ResourceDoesNotSupportOperationException();
	}
}
