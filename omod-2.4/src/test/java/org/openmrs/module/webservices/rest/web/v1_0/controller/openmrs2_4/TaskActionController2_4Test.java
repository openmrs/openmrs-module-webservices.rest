...
public class TaskActionController2_4Test extends MainResourceControllerTest {
	...
	public void setUp() throws Exception {
		...
		TaskActionResource2_4 taskActionResource = (TaskActionResource2_4) restService
	...
	public void shouldShutdownTask() throws Exception {
		...
		assertThat(mockTaskServiceWrapper.scheduledTasks, hasItem(testTask));
		deserialize(handle(
				newPostRequest(getURI(), "{\"action\": \"shutdowntask\", \"tasks\":[\"" + getTestTaskName() + "\"]}")));
		assertThat(mockTaskServiceWrapper.scheduledTasks, not(hasItem(testTask)));
	}
	...
	public void shouldRunTask() throws Exception {
		...
		deserialize(handle(
				newPostRequest(getURI(), "{\"action\": \"runtask\", \"tasks\":[\"" + getTestDummyTaskName() + "\"]}")));
		assertThat(mockTaskServiceWrapper.registeredTasks, hasItem(testDummyTask));
		Assert.assertEquals(mockTaskServiceWrapper.getRegisteredTasks().size(), 3);
