 ...
 * class hierarchy
 */
public class ClassHierarchyResourceTest extends
	...
	public void beforeEachTests() throws Exception {
		...
		resource = (OrderResource1_8) Context.getService(RestService.class)
				.getResourceBySupportedClass(Order.class);
	}

	private SimpleObject buildSuperclass() {
		return new SimpleObject().add(RestConstants.PROPERTY_FOR_TYPE, "order")
				.add("startDate", "2011-02-03").add("patient", PATIENT_UUID)
				.add("concept", ASPIRIN_CONCEPT_UUID)
				.add("orderType", LUNCH_ORDER_TYPE_UUID);
	}

	private SimpleObject buildSubclass() {
		return buildSuperclass().removeProperty("orderType")
				.add(RestConstants.PROPERTY_FOR_TYPE, "drugorder")
				.add("dose", "100").add("units", "mg").add("prn", "true")
				.add("complex", "false").add("drug", ASPIRIN_DRUG_UUID);
	}

	@Test
	@Ignore
	// TODO - database changed between 1.8 to 1.9
	public void shouldCreateASuperclass() throws Exception {
		SimpleObject created = (SimpleObject) resource.create(
				buildSuperclass(), context);
	...
	public void shouldCreateASubclass() throws Exception {
		SimpleObject created = (SimpleObject) resource.create(buildSubclass(),
				context);
	...
	public void shouldRetrieveASuperclass() throws Exception {
		SimpleObject retrieved = (SimpleObject) resource.retrieve(
				SUPERCLASS_UUID, context);
		Util.log("Retrieved superclass", retrieved);
		Assert.assertEquals("order", retrieved.get("type"));
		Assert.assertEquals("a09ab2c5-878e-4905-b25d-5784167d0216",
				Util.getByPath(retrieved, "concept/uuid"));
	}
	...
	public void shouldUpdateASuperclass() throws Exception {
		...
		Object updated = resource.update(SUPERCLASS_UUID,
				new SimpleObject().add("instructions", newValue), context);
		Util.log("Updated subclass", updated);
		Assert.assertEquals(newValue,
				PropertyUtils.getProperty(updated, "instructions"));
	}
	...
	public void shouldUpdateASubclass() throws Exception {
		Object updated = resource.update(SUBCLASS_UUID,
				new SimpleObject().add("dose", "500"), context);
		Util.log("Updated subclass", updated);
	...
	public void shouldDeleteASuperclass() throws Exception {
		...
		Order deleted = Context.getOrderService().getOrderByUuid(
				SUPERCLASS_UUID);
	...
	public void shouldPurgeASuperclass() throws Exception {
		...
		Order purged = Context.getOrderService()
				.getOrderByUuid(SUPERCLASS_UUID);
	...
	public void shouldGetAll() throws Exception {
		...
		Assert.assertTrue("drugorder".equals(typeForFirst)
				|| "order".equals(typeForFirst));
	}
	...
	@Ignore
	// TODO - database changed between 1.8 to 1.9 - URGENCY is not nullable
	public void shouldUsePropertySetterAndGetterFromSubclassHandler()
			throws Exception {
		...
		// this will only work if the @PropertySetter method on the subclass
		// handler is used
		resource.setProperty(o, "standardRegimenCode", "Peds-1a");
		// this will only work if the @PropertyGetter method on the subclass
		// handler is used
	...
	public void shouldGetAllDrugOrdersForAPatient() throws Exception {
		...
		Assert.assertEquals("drugorder",
				Util.getByPath(simple, "results[0]/type"));
	}
