...
public class PersonController1_8Test extends MainResourceControllerTest {
	...
	public void shouldGetAPersonByUuid() throws Exception {
		...
		assertEquals(person.getGender(),
				PropertyUtils.getProperty(result, "gender"));
	...
	public void shouldGetPersonDateCreated() throws Exception {
		...
		MockHttpServletRequest req = newGetRequest(getURI() + "/" + getUuid(),
				new Parameter("v", RestConstants.REPRESENTATION_FULL));
		SimpleObject result = deserialize(handle(req));

		Map<String, String> auditInfo = (Map<String, String>) PropertyUtils
				.getProperty(result, "auditInfo");

		assertEquals(ConversionUtil.convertToRepresentation(personDateCreated,
				Representation.FULL), auditInfo.get("dateCreated"));
	}

	@Test
	public void shouldReturnTheAuditInfoForTheFullRepresentation()
			throws Exception {
		MockHttpServletRequest req = request(RequestMethod.GET, getURI() + "/"
				+ getUuid());
	...
	public void shouldCreateAPerson() throws Exception {
		...
		SimpleObject newPerson = deserialize(handle(newPostRequest(getURI(),
				json)));
	...
	public void shouldEditAPerson() throws Exception {
		...
		SimpleObject response = deserialize(handle(newPostRequest(getURI()
				+ "/" + getUuid(), json)));
		assertNotNull(response);
		Object responsePersonContents = PropertyUtils.getProperty(response,
				"person");
		assertNotNull(responsePersonContents);
		assertTrue("F".equals(PropertyUtils.getProperty(responsePersonContents,
				"gender").toString()));
	...
	@Test
	public void shouldSetThePreferredAddressAndUnmarkTheOldOne()
			throws Exception {
		executeDataSet("PersonControllerTest-otherPersonData.xml");
		Person person = service.getPersonByUuid(getUuid());
		PersonAddress preferredAddress = service
				.getPersonAddressByUuid("8a806d8c-822d-11e0-872f-18a905e044dc");
		PersonAddress notPreferredAddress = service
				.getPersonAddressByUuid("3350d0b5-821c-4e5e-ad1d-a9bce331e118");
	...
	public void shouldSetThePreferredNameAndUnmarkTheOldOne() throws Exception {
		executeDataSet("PersonControllerTest-otherPersonData.xml");
		Person person = service.getPersonByUuid(getUuid());
		PersonName preferredName = service
				.getPersonNameByUuid("399e3a7b-6482-487d-94ce-c07bb3ca3cc7");
		PersonName notPreferredName = service
				.getPersonNameByUuid("499e3a7b-6482-487d-94ce-c07bb3ca3cc8");
	...
	public void shouldVoidAPerson() throws Exception {
		...
		MockHttpServletRequest req = newDeleteRequest(getURI() + "/"
				+ getUuid(), new Parameter("!purge", ""), new Parameter(
				"reason", reason));
		handle(req);
	...
	public void shouldPurgeAPerson() throws Exception {
		...
		MockHttpServletRequest req = newDeleteRequest(getURI() + "/" + uuid,
				new Parameter("purge", ""));
		handle(req);
	...
	@Test
	public void shouldSearchAndReturnAListOfPersonsMatchingTheQueryString()
			throws Exception {
		MockHttpServletRequest req = request(RequestMethod.GET, getURI());
		...
		assertEquals(getUuid(), PropertyUtils.getProperty(
				Util.getResultsList(result).get(0), "uuid"));
	}
	...
	public void shouldNotShowVoidedNamesInFullRepresentation() throws Exception {
		...
		if (!nameToVoid.isVoided()) {
			// void the Name
			handle(newDeleteRequest("person/" + getUuid() + "/name/"
					+ nameToVoidUuid, new Parameter("!purge", ""),
		...
		List<SimpleObject> names = (List<SimpleObject>) PropertyUtils
				.getProperty(result, "names");
		assertEquals(1, names.size());
		assertFalse(nameToVoidUuid.equals(PropertyUtils.getProperty(
				names.get(0), "uuid")));
	}

	@SuppressWarnings("unchecked")
	@Test
	public void shouldNotShowVoidedAddressesInFullRepresentation()
			throws Exception {
		executeDataSet("PersonControllerTest-otherPersonData.xml");
		Person person = service.getPersonByUuid(getUuid());
		assertEquals(2, person.getAddresses().size());
		PersonAddress voidedAddress = service
				.getPersonAddressByUuid("8a806d8c-822d-11e0-872f-18a905e044dc");
		...
		List<SimpleObject> addresses = (List<SimpleObject>) PropertyUtils
				.getProperty(result, "addresses");
		assertEquals(1, addresses.size());
		assertFalse(voidedAddressUuid.equals(PropertyUtils.getProperty(
				addresses.get(0), "uuid")));
	}

	@SuppressWarnings("unchecked")
	@Test
	public void shouldNotShowVoidedAttributesInFullRepresentation()
			throws Exception {
		Person person = service.getPersonByUuid(getUuid());
		...
		if (!attributeToVoid.isVoided()) {
			// void the attribute
			handle(newDeleteRequest("person/" + getUuid() + "/attribute/"
					+ attributeToVoidUuid, new Parameter("!purge", ""),
					new Parameter("reason", "none")));
		}
		...
		List<SimpleObject> attributes = (List<SimpleObject>) PropertyUtils
				.getProperty(result, "attributes");
		assertEquals(2, attributes.size());
		List<Object> uuids = Arrays.asList(
