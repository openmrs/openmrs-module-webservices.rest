...
public class EncounterController1_8Test extends MainResourceControllerTest {
	...
	public static final String currentTimezone = Calendar.getInstance()
			.getTimeZone().getDisplayName(true, TimeZone.SHORT);
	 ...
	 */
	@Test
	public void createEncounter_shouldCreateANewEncounterUsingEncounterTypeRef()
			throws Exception {
		long before = getAllCount();
	...
	@Test
	public void createEncounter_shouldCreateANewEncounterUsingPatientRef()
			throws Exception {
	...
	@Test
	public void createEncounter_shouldDefaultDatetimeToNowIfNotSpecified()
			throws Exception {
		Date since = new Date();
		...
		Object newEncounter = deserialize(handle(newPostRequest(getURI(), json)));
		Assert.assertNotNull(newEncounter);
		Date encounterDatetime = (Date) ConversionUtil.convert(
				((SimpleObject) newEncounter).get("encounterDatetime"),
				Date.class);
		Assert.assertTrue(encounterDatetime.compareTo(since) >= 0);
	}

	@Test
	public void createEncounter_shouldCreateANewEncounterWithEmptyUnitOnNumericConcept()
			throws Exception {
	...
	private SimpleObject encounterWithObs() throws Exception {
		...
		obs.add(SimpleObject
				.parseJson("{ \"concept\": \"c607c80f-1ea9-4da3-bb88-6276ce8868dd\", \"value\": 70 }"));
		...
		return new SimpleObject()
				...
				.add("provider", "ba1b19c2-3ed6-4f63-b8c0-f762dc8d7562")
				.add("obs", obs);
	}
	...
	@Test
	@Ignore
	// TODO - unexpected exception type
	public void createEncounter_shouldCreateANewEncounterWithObs()
			throws Exception {
		long before = getAllCount();
		SimpleObject post = encounterWithObs();

		SimpleObject newEncounter = deserialize(handle(newPostRequest(getURI(),
				post)));
		...
		Assert.assertTrue(obsDisplayValues
				.contains("FAVORITE FOOD, NON-CODED: fried chicken"));
		Assert.assertTrue(obsDisplayValues.contains("WEIGHT (KG): 70.0"));

		// obs.getValueAsString() uses application Locale and hence have to do
		// this
		Calendar cal = Calendar.getInstance();
		cal.set(2011, Calendar.JUNE, 21, 0, 0, 0);
		String format = Format.format(cal.getTime(), Context.getLocale(),
				FORMAT_TYPE.TIMESTAMP);
		Assert.assertTrue(obsDisplayValues.contains("DATE OF FOOD ASSISTANCE: "
				+ format));
	}
	...
	@Ignore
	// TODO - unexpected exception type
	public void shouldCreateAnEncounterWithObsAndOrdersOfDifferentTypes()
			throws Exception {
		...
		orders.add(SimpleObject
				.parseJson("{ \"type\": \"order\", \"concept\": \""
						+ foodAssistanceUuid + "\", \"orderType\": \""
						+ lunchOrderUuid + "\", \"instructions\": \""
						+ lunchInstructions + "\" }"));
		orders.add(SimpleObject
				.parseJson("{ \"type\": \"drugorder\", \"concept\": \""
						+ triomuneConceptUuid + "\", \"drug\": \""
						+ triomuneDrugUuid
						+ "\", \"dose\": \"1\", \"units\": \"tablet\" }"));
		...
		List<String> lookFor = new ArrayList<String>(Arrays.asList(
				"FOOD ASSISTANCE", "Triomune-30: 1.0 tablet"));
		for (Map o : newOrders) {
	 ...
	 */
	@Test
	public void findEncounters_shouldReturnNoResultsIfThereAreNoMatchingEncounters()
			throws Exception {
		SimpleObject object = deserialize(handle(newGetRequest(getURI(),
				new Parameter("q", "noencounter"))));
	...
	public void getEncounter_shouldGetAFullRepresentationOfAEncounterIncludingObsGroups()
		...
		Assert.assertEquals("Some text",
				Util.getByPath(result, "obs[0]/groupMembers[0]/value"));
		Assert.assertEquals("11716f9c-1434-4f8d-b9fc-9aa14c4d6126",
				Util.getByPath(result, "obs[0]/groupMembers[1]/concept/uuid"));
		Assert.assertEquals(ConversionUtil.convertToRepresentation(
				ymd.parse("2011-06-12"), Representation.DEFAULT), Util
		...
		Assert.assertEquals("96408258-000b-424e-af1a-403919332938", Util
				.getByPath(result,
						"obs[0]/groupMembers[2]/groupMembers[0]/concept/uuid"));
		Assert.assertEquals("Some text", Util.getByPath(result,
				"obs[0]/groupMembers[2]/groupMembers[0]/value"));
		Assert.assertEquals("11716f9c-1434-4f8d-b9fc-9aa14c4d6126", Util
				.getByPath(result,
						"obs[0]/groupMembers[2]/groupMembers[1]/concept/uuid"));
		Assert.assertEquals(ConversionUtil.convertToRepresentation(
				ymd.parse("2011-06-12"), Representation.DEFAULT), Util
				.getByPath(result,
						"obs[0]/groupMembers[2]/groupMembers[1]/value"));
	}
	...
	public void purgeEncounter_shouldNotPurgeAEncounterWithDependentData()
			throws Exception {
		int size = Context.getEncounterService()
				.getEncountersByPatient(new Patient(7)).size();
		handle(newDeleteRequest(getURI()
				+ "/6519d653-393b-4118-9c83-a3715b82d4ac", new Parameter(
				"purge", "")));

		Assert.assertEquals(size - 1, Context.getEncounterService()
				.getEncountersByPatient(new Patient(7)).size());
	}
	 ...
	 */
	@Test
	public void updateEncounter_shouldChangeAPropertyOnAEncounter()
			throws Exception {
		Date now = new Date();
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		SimpleObject post = new ObjectMapper().readValue(
				"{\"encounterDatetime\":\"" + df.format(now) + "\"}",
				SimpleObject.class);
		handle(newPostRequest(getURI()
				+ "/6519d653-393b-4118-9c83-a3715b82d4ac", post));
		Assert.assertEquals(
				df.format(now),
				df.format(Context.getEncounterService().getEncounter(3)
	 ...
	 */
	@Test
	public void searchByPatient_shouldReturnAListOfEncounters()
			throws Exception {
		MockHttpServletRequest req = new MockHttpServletRequest();
		req.addParameter(RestConstants.REQUEST_PROPERTY_FOR_REPRESENTATION,
				RestConstants.REPRESENTATION_FULL);

		Object result = deserialize(handle(newGetRequest(
				getURI(),
		...
		Assert.assertTrue(uuids
				.contains("6519d653-393b-4118-9c83-a3715b82d4ac"));
	}

	@Test
	public void search_shouldSearchForEncountersByASearchPhrase()
			throws Exception {
		SimpleObject response = deserialize(handle(newGetRequest(getURI(),
				new Parameter("q", "Collet"))));
	...
	public void shouldGetEncountersByEncounterTypeAndPatient() throws Exception {
		...
		SimpleObject result = deserialize(handle(newGetRequest(getURI(),
				new Parameter("s", "default"), new Parameter("patient",
						"41c6b35e-c093-11e3-be87-005056821db0"),
				new Parameter("encounterType",
		...
		String encounterUuid = (String) PropertyUtils.getProperty(
				encounters.get(0), "uuid");
		Assert.assertEquals("62967e68-96bb-11e0-8d6b-9b9415a91465",
				encounterUuid);
	}
	...
	public long getAllCount() {
		Map<Integer, List<Encounter>> allPatientEncounters = Context
				.getEncounterService().getAllEncounters(null);
