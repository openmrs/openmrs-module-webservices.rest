...
public class ObsResource1_8Test extends
		BaseDelegatingResourceTest<ObsResource1_8, Obs> {
	...
	public void asRepresentation_shouldReturnProperlyEncodedValues()
			throws Exception {
		Obs obs = getObject();
		obs.setComment(null); // to test that we don't get a NPE when no comment
								// (specifically with the Location example)
		...
		rep = getResource().asRepresentation(getObject(),
				Representation.DEFAULT);
		Assert.assertEquals("datetime", datetime,
				ConversionUtil.convert(rep.get("value"), Date.class));

		// drug
		Drug drug = Context.getConceptService().getDrugByUuid(
	...
	public void getGroupMembers_shouldReturnAllGroupMembers() throws Exception {
		...
		Set<Obs> groupMembers1 = (Set<Obs>) ObsResource1_8
				.getGroupMembers(resource
						.getByUniqueId("47f18998-96cc-11e0-8d6b-9b9415a91423"));
		assertEquals(1, groupMembers1.size());

		Set<Obs> groupMembers2 = (Set<Obs>) ObsResource1_8
				.getGroupMembers(resource
						.getByUniqueId("5117f5d4-96cc-11e0-8d6b-9b9415a91433"));
	...
	public void setValue_shouldThrowExceptionOnUnexpectedValue()
			throws Exception {
		Obs obs = new Obs();
		obs.setConcept(Context.getConceptService().getConceptByUuid(
				BOOLEAN_CONCEPT_UUID));
	...
	private void clearAndSetValue(Obs obs, ObsType type, Object value) {
		...
		obs.setValueComplex(type.equals(ObsType.COMPLEX) ? (String) value
				: null);
		obs.setValueDatetime(type.equals(ObsType.DATETIME) ? (Date) value
				: null);
		obs.setValueDrug(type.equals(ObsType.DRUG) ? (Drug) value : null);
		obs.setValueNumeric(type.equals(ObsType.NUMERIC) ? (Double) value
				: null);
		obs.setValueText(type.equals(ObsType.TEXT) ? (String) value : null);
	}

	@Test
	public void setConvertedProperties_shouldAllowAnyPropertyOrder()
			throws Exception {
		ObsResource1_8 resource = getResource();
		Obs obs = getObject();
		...
		resource.setConvertedProperties(obs, propertyMap,
				resource.getUpdatableProperties(), false);
		org.springframework.util.Assert.isTrue(((Double) ObsResource1_8
				.getValue(obs)) == 10.0);
	}
}
