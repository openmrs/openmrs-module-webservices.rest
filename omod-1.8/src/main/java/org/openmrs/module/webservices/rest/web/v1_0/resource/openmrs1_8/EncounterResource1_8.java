 ...
 */
@Resource(name = RestConstants.VERSION_1 + "/encounter", supportedClass = Encounter.class, supportedOpenmrsVersions = "1.9.*", order = 200)
public class EncounterResource1_8 extends DataDelegatingCrudResource<Encounter> {
	...
	public Encounter newDelegate() {
		...
		// default to now(), so a web client can create a real-time encounter
		// based on the server time
		enc.setEncounterDatetime(new Date());
		// As of 2012-04-27 there is a bug in Encounter.getOrders() where, if
		// null, it returns an empty list without keeping a reference to it
	 ...
	 */
	@Override
	public void delete(Encounter enc, String reason, RequestContext context)
	 ...
	 */
	@Override
	public void purge(Encounter enc, RequestContext context)
			throws ResponseException {
		if (enc == null) {
			// DELETE is idempotent, so we return success here
			return;
	...
	public String getDisplayString(Encounter encounter) {
		String ret = encounter.getEncounterType() == null ? "?" : encounter
				.getEncounterType().getName();
		ret += " ";
		ret += encounter.getEncounterDatetime() == null ? "?" : Context
				.getDateFormat().format(encounter.getEncounterDatetime());
		return ret;
	}
	 ...
	 * {@link RequestContext} otherwise searches for encounters that match the
	 * specified query
	 * 
	 * @param context
	...
	protected PageableResult doSearch(RequestContext context) {
		...
		return new ServiceSearcher<Encounter>(EncounterService.class,
				"getEncounters", "getCountOfEncounters").search(
