...
@Resource(name = RestConstants.VERSION_1 + "/relationshiptype", order = 200, supportedClass = RelationshipType.class, supportedOpenmrsVersions = {
		"1.9.*", "1.10.*", "1.11.*", "1.12.*" })
public class RelationShipTypeResource1_8 extends
		MetadataDelegatingCrudResource<RelationshipType> {

	/**
	 * Fetches a relationshipType by uuid, if no match is found, it tries to
	 * look up one with a matching name with the assumption that the passed
	 * parameter is a relationshipType name
	 *
	...
	public RelationshipType getByUniqueId(String uuid) {
		...
		if (relationshipType == null) {
			List<RelationshipType> relationshipTypes = Context
					.getPersonService().getAllRelationshipTypes();
			for (RelationshipType possibleRelationshipType : relationshipTypes) {
				if (possibleRelationshipType.getaIsToB().equalsIgnoreCase(uuid)
						|| possibleRelationshipType.getbIsToA()
	 ...
	 */
	@Override
	public void purge(RelationshipType relationshipType, RequestContext context)
	...
	protected NeedsPaging<RelationshipType> doGetAll(RequestContext context) {
		...
		return new NeedsPaging<RelationshipType>(unRetiredRelationshipTypes,
				context);
	}
	...
	protected NeedsPaging<RelationshipType> doSearch(RequestContext context) {
		...
		List<RelationshipType> allRelationshipTypes = Context
				.getPersonService().getAllRelationshipTypes();
		List<RelationshipType> unRetiredRelationshipTypes = new ArrayList<RelationshipType>();
		for (RelationshipType relationshipType : allRelationshipTypes) {
			if (!relationshipType.isRetired()
					&& (relationshipType.getaIsToB().contains(queryString) || relationshipType
							.getbIsToA().contains(queryString)))
				unRetiredRelationshipTypes.add(relationshipType);
		}
		return new NeedsPaging<RelationshipType>(unRetiredRelationshipTypes,
				context);
