...
/**
 * {@link Resource} for {@link EncounterType}, supporting standard CRUD
 * operations
 */
...
public class EncounterTypeResource1_8 extends
		MetadataDelegatingCrudResource<EncounterType> {
	...
	/**
	 * Fetches a encounterType by uuid, if no match is found, it tries to look
	 * up one with a matching name with the assumption that the passed parameter
	 * is a encounterType name
	 * 
	 ...
	 */
	@Override
	public void purge(EncounterType encounterType, RequestContext context)
	...
	protected NeedsPaging<EncounterType> doGetAll(RequestContext context) {
		return new NeedsPaging<EncounterType>(Context.getEncounterService()
				.getAllEncounterTypes(context.getIncludeAll()), context);
	}
	...
	protected NeedsPaging<EncounterType> doSearch(RequestContext context) {
		return new NeedsPaging<EncounterType>(Context.getEncounterService()
				.findEncounterTypes(context.getParameter("q")), context);
	}
}
