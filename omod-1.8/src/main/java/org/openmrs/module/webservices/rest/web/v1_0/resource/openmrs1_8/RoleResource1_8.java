 ...
 */
@Resource(name = RestConstants.VERSION_1 + "/role", order = 200, supportedClass = Role.class, supportedOpenmrsVersions = {
		"1.9.*", "1.10.*", "1.11.*", "1.12.*" })
public class RoleResource1_8 extends MetadataDelegatingCrudResource<Role> {
	 ...
	 */
	@Override
	public void purge(Role delegate, RequestContext context)
	...
	public DelegatingResourceDescription getRepresentationDescription(
		...
		if (rep instanceof DefaultRepresentation) {
			...
			description
					.addProperty("allInheritedRoles", Representation.DEFAULT);
	...
	public static String getRoleName(Role role) {
		return role.getRole(); // The role "name" is actually stored in the role
								// property.
	}
	...
	public static void setRoleName(Role role, Object name) {
		role.setRole((String) name); // The role "name" is actually stored in
										// the role property.
	}
	...
	@Override
	protected NeedsPaging<Role> doGetAll(RequestContext context)
			throws ResponseException {
		return new NeedsPaging<Role>(Context.getUserService().getAllRoles(),
				context);
	}
